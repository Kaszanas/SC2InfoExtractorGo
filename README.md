...
- **Imperative install (single user):**
  ```sh
  nix profile install github:Kaszanas/SC2InfoExtractorGo
  ```
- **Declarative — flake-based NixOS `configuration.nix`:** add as a flake input and reference the package in `environment.systemPackages`:
  ```nix
  # flake.nix
  inputs.sc2infoextractorgo.url = "github:Kaszanas/SC2InfoExtractorGo";

  # configuration.nix
  environment.systemPackages = [ inputs.sc2infoextractorgo.packages.${pkgs.system}.default ];
  ```
- **Declarative — home-manager:** same input, referenced from `home.packages`:
  ```nix
  home.packages = [ inputs.sc2infoextractorgo.packages.${pkgs.system}.default ];
  ```
- **Non-flake NixOS (`nix-channel`-based configs):**
  ```nix
  environment.systemPackages = [
    (builtins.getFlake "github:Kaszanas/SC2InfoExtractorGo").packages.${builtins.currentSystem}.default
  ];
  ```
